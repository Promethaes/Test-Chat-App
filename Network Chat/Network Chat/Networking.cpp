...
Network::Network(unsigned port, const std::string& IP, bool isHosting)
{
...
void Network::listen()
{
	...
	if (_isHosting) {
		...
		for (unsigned i = 0; i < connectedIPs.size(); i++) {
			if (connectedIPs[i] == temp)
				alreadyConnected = true;
		}
		if (!alreadyConnected)
			connectedIPs.push_back(temp);

		for (unsigned i = 0; i < connectedIPs.size(); i++) {
			if (connectedIPs[i] != temp)
				sendMessage(buf, connectedIPs[i]);
		}
	}
